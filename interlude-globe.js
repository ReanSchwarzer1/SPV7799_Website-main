    ...
    window.Interludes.register("globe", {
      ...
      build: function (ctx) {
        ...
        var halo = new THREE.Mesh(
          keep(new THREE.SphereGeometry(A.globe.r * 1.035, 64, 40)),
        ...
        for (var la = -60; la <= 60; la += 30) {
          ...
          var ring = new THREE.Mesh(keep(new THREE.TorusGeometry(rr, 0.012, 32, 128)), wireMat);
          ring.rotation.x = Math.PI / 2;
          ring.position.y = A.globe.r * Math.sin(la * Math.PI / 180);
          globe.add(ring);
        }
        for (var lo = 0; lo < 180; lo += 30) {
          var mer = new THREE.Mesh(keep(new THREE.TorusGeometry(A.globe.r, 0.012, 32, 128)), wireMat);
        ...
        function makeMarker(lat, lng, color, big) {
          var m = new THREE.Mesh(
            keep(new THREE.SphereGeometry(big ? 0.2 : 0.16, 64, 40)),
          ...
          var halo2 = new THREE.Mesh(
            keep(new THREE.TorusGeometry(big ? 0.34 : 0.28, 0.028, 32, 128)),
          ...
          var hit = new THREE.Mesh(
            keep(new THREE.SphereGeometry(0.52, 64, 40)),
        ...
        A.dests.forEach(function (d) {
          ...
          var tube = new THREE.Mesh(
            keep(new THREE.TubeGeometry(curve, 128, 0.05, 24, false)),
          ...
          var ship = new THREE.Mesh(
            keep(new THREE.SphereGeometry(0.11, 64, 40)),
        ...
        var apiTube = new THREE.Mesh(
          keep(new THREE.TubeGeometry(new THREE.QuadraticBezierCurve3(cFrom, cMid, cTo), 128, 0.035, 24, false)),
        ...
        var knob = new THREE.Mesh(
          keep(new THREE.SphereGeometry(A.rail.knob, 64, 40)),
